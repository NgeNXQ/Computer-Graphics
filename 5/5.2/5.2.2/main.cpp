...
int main(void)
{
    ...
    const double VIEWPORT_SCALER = 1.0;
    ...
    GLFWwindow* const window = glfwCreateWindow(VIEWPORT_INITIAL_WIDTH, VIEWPORT_INITIAL_HEIGHT, "Labwork #5 (5.2.2). IP-14 Babich Denys", NULL, NULL);
...
void DrawPanoram(GLuint textureID, float angle)
{
    ...
    for (int i = 0; i <= n; ++i)
    {
        glTexCoord2d(i / (float)n + s, 1);
        glVertex3f(r * cos(fi), r * sin(fi), h / 2);

        glTexCoord2d(i / (float)n + s, 0);
        glVertex3f(r * cos(fi), r * sin(fi), -h / 2);
