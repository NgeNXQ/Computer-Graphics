...
void DrawCylinder(const int, const float, const float);

int main(void)
{
    const int VIEWPORT_X_OFFSET = 0;
    const int VIEWPORT_Y_OFFSET = 0;
    const double VIEWPORT_SCALER = 8.5;
    ...
    const int PRISM_SIDES_COUNT = 20;
    ...
    GLFWwindow* const window = glfwCreateWindow(VIEWPORT_INITIAL_WIDTH, VIEWPORT_INITIAL_HEIGHT, "Labwork #3 (3.4). IP-14 Babich Denys", NULL, NULL);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        DrawCylinder(CYLINDER_SIDES_COUNT, CYLINDER_HEIGHT, CYLINDER_RADIUS_CIRCUMCIRCLE_BASE);
...
void DrawCylinder(const int sidesCount, const float height, const float radiusCircumcircleBase)
{
    const float DELTA_FI = 2.0f * M_PI / sidesCount;
